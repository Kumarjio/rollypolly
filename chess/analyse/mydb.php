...
if (empty($_GET['side'])) {
  throw new Exception('empty side');
}
if (empty($_GET['base_id'])) {
  throw new Exception('empty base_id');
}
...
$queryChessCurrentMove = "SELECT * FROM chess_repertorie_moves WHERE uid = ? AND repertory_id = ? AND move_id >= ? AND flag = 0 LIMIT 1";
$resultChessCurrentMove = $modelGeneral->fetchRow($queryChessCurrentMove, array($_SESSION['user']['id'], $_GET['id'], $_GET['base_id']), 0);
...
if (!empty($legalMoves['success'])) {
  ...
  exit;
  $legalMoves = array();
} else if ($resultChessCurrentMove['move_side'] == $_GET['side']) {
  pr($legalMoves);
  if (!empty($legalMoves)) {
