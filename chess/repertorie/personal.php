...
if (!empty($_GET['move'])) {
    ...
    if (empty($resultCheck)) {
        ...
        header("Location: ".HTTPPATH."/chess/repertorie/$pageUrl?id=".$_GET['id']."&move_id=".$result."&parent_id=".$result."&act=ana");
        ...
        if (!empty($arrSelectedDetails)) {
          ...
          foreach ($arrSelectedDetails as $k => $v) {
            if ($v['details']['move_side'] === 'W') {
              ...
              $pgn .= $v['details']['move_number'].'. ';
            }
            $string .= '<a href="'.HTTPPATH.'/chess/repertorie/'.$pageUrl.'?id='.$_GET['id'].'&move_id='.$v['details']['move_id'].'&parent_id='.$v['details']['move_id'].'" class="acustom">'.$v['details']['move'].'</a> ';
            $pgn .= $v['details']['move'].' ';
          }
          $string .= '<br>';
          $string .= $pgn;
...
function lozUpdateBestMove ()
{
  ...
  chess.load_pgn('<?php echo $pgn; ?>');
  console.log(chess);
  var pretty_move = chess.move(move);
  console.log(move);
  console.log(pretty_move.san);
  console.log(chess.fen());
  var url = '<?php echo HTTPPATH; ?>/chess/repertorie/<?php echo $pageUrl; ?>?id=<?php echo $_GET['id']; ?>&move='+encodeURIComponent(pretty_move.san)+'&fen='+encodeURIComponent(chess.fen())+'&level=<?php echo $level; ?>&move_id=<?php echo $move_id; ?>&parent_id=<?php echo $parent_id; ?>&toMove=<?php echo $toMove; ?>';
  console.log(url);
