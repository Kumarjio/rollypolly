...
class Horo2 extends MY_Controller
{

	public function __construct()
	{
		parent::__construct();
		$this->layout->setLayout('layout_mm');
		$this->_login2();
	...
	{
		$data = array('menuItemSel' => '', 'title' => 'AstroMatch');
		$this->load->view('kundali2/intro', $data);
	}

	public function home()
	{
		$data = array('menuItemSel' => '', 'title' => 'AstroMatch');
		$this->layout->view('kundali2/pages', $data);
	}

	public function user()
	{
		$data = array('menuItemSel' => '');
		$this->layout->view('kundali2/login', $data);
	}
	...
	{
		$data = array('menuItemSel' => 'matchmakingprofile', 'title' => 'AstroMatch::Match Making Profile');
		try {
		...
		$this->layout->view('kundali2/matchmakingprofile', $data);
	}
	...
	public function matchmakingprofilepost()
	{
		...
		try {
			...
			$html = $this->layout->view('kundali2/matchmakingprofile_list', $data, true);
	...
	{
		$ret = array('menuItemSel' => 'mylocation', 'title' => 'AstroMatch::Set Location');
		$this->layout->view('kundali2/location', $ret);
	}
	...
	public function locationset()
	{
		...
		$ret = array_merge($ret, array('menuItemSel' => 'mylocation'));
		$this->json_($ret);
		//header('content-type: application/json; charset=utf-8');
	...
	public function mylocation()
	{
		...
		$ret = array_merge($ret, array('menuItemSel' => 'mylocation', 'title' => 'AstroMatch::My Locations'));
		$this->layout->view('kundali2/mylocation', $ret);
	}
	...
	{
		$data = array('menuItemSel' => 'mybirthdetails', 'title' => 'AstroMatch::My Birth Details');
		try {
			$user_id = $_COOKIE['user_id'];
			$this->load->model('Horo_Model');
			$ret = $this->Horo_Model->get_location($user_id);
			...
			$data['birthprofileview'] = $this->load->view('kundali2/birthdetails_list', $data, true);
		} catch (Exception $e) {
			
		}
		$this->layout->view('kundali2/mybirthdetails', $data);
	}
	...
	public function mybirthdetailspost()
	{
		...
		try {
			...
			$data['records'] = $this->Horo_Model->get_birth_profile($user_id);
			$html = $this->layout->view('kundali2/birthdetails_list', $data, true);
			$this->json_(array('error' => 0, 'error_message' => '', 'html' => $html));
		} catch (Exception $e) {
			$this->json_(array('error' => 1, 'error_message' => $e->getMessage(), 'code' => $e->getCode()));
		}
	...
	{
		$data = array('menuItemSel' => 'bestmatches', 'title' => 'AstroMatch::Best Matches');
		try {
			$user_id = $_COOKIE['user_id'];
			$this->load->model('Horo_Model');
			$data['profiles'] = $this->Horo_Model->get_birth_profile($user_id);
		...
		$this->layout->view('kundali2/bestmatches', $data);
	}
	...
	public function bestmatchespost()
	{
		...
		try {
			...
			$html = $this->layout->view('kundali2/bestmatches_list', $data, true);
			$this->json_(array('error' => 0, 'error_message' => '', 'html' => $html));
		} catch (Exception $e) {
			$this->json_(array('error' => 1, 'error_message' => $e->getMessage(), 'code' => $e->getCode()));
		}
	...
	{
		$data = array('menuItemSel' => 'dailymatches', 'title' => 'AstroMatch::Daily Matches');
		try {
			$user_id = $_COOKIE['user_id'];
			$this->load->model('Horo_Model');
			$data['profiles'] = $this->Horo_Model->get_birth_profile($user_id);
		...
		$this->layout->view('kundali2/dailymatches', $data);
	}
	...
	public function dailymatchespost()
	{
		...
		try {
			...
			$html .= $this->layout->view('kundali2/dailymatches_list', $data, true);
			$this->json_(array('error' => 0, 'error_message' => '', 'html' => $html));
		} catch (Exception $e) {
			$this->json_(array('error' => 1, 'error_message' => $e->getMessage(), 'code' => $e->getCode()));
		}
	...
	{
		$data = array('menuItemSel' => 'history');
		$this->load->model('Horo_Model');
		$data['result'] = $this->Horo_Model->get_history();
		$this->layout->view('kundali2/history', $data);
	}
}