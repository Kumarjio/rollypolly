<?php
class PaginateIt {
    var $currentPage, $itemCount, $itemsPerPage, $linksHref, $linksToDisplay, $divtag;
        ...
        if(isset($start)){
            if($start > 1){
                if(!empty($this->pageJumpBack)){
                    ...
                    $strLinks .= '<a href="#" onClick=\'doAjaxLoadingText("'.$this->linksHref.'","GET","'.$queryString.$pageNum.'", "", "'.$this->divtag.'", "yes", "Loading...","0","","","","","0"); \'>';
                    $strLinks .= $this->pageJumpBack.'</a>'.$this->pageSeparator;
                }
				
				$strLinks .= "<a href='#' onClick=\"doAjaxLoadingText('".$this->linksHref."','GET','".$queryString.($this->currentPage-1)."', '', '".$this->divtag."', 'yes', 'Loading...','0','','','','','0'); \">Previous</a>".$this->pageSeparator; // added
				$tmpStart=1;// added
                $strLinks .= '<a href="#" onClick=\'doAjaxLoadingText("'.$this->linksHref.'","GET","'.$queryString.'1", "", "'.$this->divtag.'", "yes", "Loading...","0","","","","","0"); \'>1...</a>'.$this->pageSeparator;
            }
            ...
            for($i = $start; $i <= $end; $i ++){
                if($i != $this->currentPage){
                    $strLinks .= '<a href="#" onClick=\'doAjaxLoadingText("'.$this->linksHref.'","GET","'.$queryString.($i).'", "", "'.$this->divtag.'", "yes", "Loading...","0","","","","","0"); \'>';
                    $strLinks .= ($i).'</a>'.$this->pageSeparator;
                }
                else {
                    $strLinks .= $i.$this->pageSeparator;
			...
			if(($this->currentPage)!=$pageCount && ($start + $this->linksToDisplay - 1 < $pageCount)!=1) {// added
				$strLinks .= $this->pageSeparator."<a href='#' onClick=\"doAjaxLoadingText('".$this->linksHref."','GET','".$queryString.($this->currentPage+1)."', '', '".$this->divtag."', 'yes', 'Loading...','0','','','','','0'); \">Next</a>"; // added
			} // added

            if($start + $this->linksToDisplay - 1 < $pageCount){
                $strLinks .= $this->pageSeparator.'<a href="#" onClick=\'doAjaxLoadingText("'.$this->linksHref.'","GET","'.$queryString.$pageCount.'", "", "'.$this->divtag.'", "yes", "Loading...","0","","","","","0"); \'>';
                $strLinks .= '...'.$pageCount.'</a>'.$this->pageSeparator;
                
				$strLinks .= "<a href='#' onClick=\"doAjaxLoadingText('".$this->linksHref."','GET','".$queryString.($this->currentPage+1)."', '', '".$this->divtag."', 'yes', 'Loading...','0','','','','','0'); \">Next</a>".$this->pageSeparator; // added
	...
	function GetPageLinksComplete(){
        $strLinks = '';
        $pageCount = $this->GetPageCount();
        $queryString = $this->GetQueryString_Complete();
        $linksPad = floor($this->linksToDisplay/2);
        ...
        if(isset($start)){
            if($start > 1){
                if(!empty($this->pageJumpBack)){
                    ...
                    $strLinks .= '<a href="'.$this->linksHref.'?'.$queryString.$pageNum.'" onClick=\'openGetUrl("'.$this->linksHref.'", "'.$this->divtag.'", "'.$queryString.$pageNum.'"); return false;\'>';
                    $strLinks .= $this->pageJumpBack.'</a>'.$this->pageSeparator;
                }
				
				$strLinks .= "<a href='".$this->linksHref.'?'.$queryString.($this->currentPage-1)."' onClick=\"openGetUrl('".$this->linksHref."', '".$this->divtag."', '".$queryString.($this->currentPage-1)."'); return false;\">Previous</a>".$this->pageSeparator; // added
				$tmpStart=1;// added
                $strLinks .= '<a href="'.$this->linksHref.'?'.$queryString.'1" onClick=\'openGetUrl("'.$this->linksHref.'", "'.$this->divtag.'", "'.$queryString.'1"); return false;\'>1...</a>'.$this->pageSeparator;
            }
            ...
            for($i = $start; $i <= $end; $i ++){
                if($i != $this->currentPage){
                    $strLinks .= '<a href="'.$this->linksHref.'?'.$queryString.($i).'" onClick=\'openGetUrl("'.$this->linksHref.'", "'.$this->divtag.'", "'.$queryString.($i).'"); return false;\'>';
                    $strLinks .= ($i).'</a>'.$this->pageSeparator;
                }
                else {
                    $strLinks .= $i.$this->pageSeparator;
			...
			if(($this->currentPage)!=$pageCount && ($start + $this->linksToDisplay - 1 < $pageCount)!=1) {// added
				$strLinks .= $this->pageSeparator."<a href='".$this->linksHref.'?'.$queryString.($this->currentPage+1)."' onClick=\"openGetUrl('".$this->linksHref."', '".$this->divtag."', '".$queryString.($this->currentPage+1)."'); return false;\">Next</a>"; // added
			} // added

            if($start + $this->linksToDisplay - 1 < $pageCount){
                $strLinks .= $this->pageSeparator.'<a href="'.$this->linksHref.'?'.$queryString.$pageCount.'" onClick=\'openGetUrl("'.$this->linksHref.'", "'.$this->divtag.'", "'.$queryString.$pageCount.'"); return false;\'>';
                $strLinks .= '...'.$pageCount.'</a>'.$this->pageSeparator;
                
				$strLinks .= "<a href='".$this->linksHref.'?'.$queryString.($this->currentPage+1)."' onClick=\"openGetUrl('".$this->linksHref."', '".$this->divtag."', '".$queryString.($this->currentPage+1)."'); return false;\">Next</a>".$this->pageSeparator; // added
	...
	function GetQueryString_Complete(){
		$pattern = array('/'.$this->queryStringVar.'=[^&]*&?/', '/&$/');
        $replace = array('', '');
        $queryString = preg_replace($pattern, $replace, $this->queryString);
        $queryString = str_replace('&', '&amp;', $queryString);
