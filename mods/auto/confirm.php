  ...
  if (!empty($_GET['error'])) {
    $error = $_GET['error_description'];
      ...
      if (verifyGatewaySignature($d['signature'], $d['checkoutId'], $d['amount'])) {
          ...
          header("Location: ".$currentURL."/auto/confirm?id=".$_GET['id']."&module_id=".$colname_rsModule."&msg=".urlencode($error));
          exit;
      }//end verify if
  }//end error if
}
...
?>
<div class="jumbotron">
  <h1>Confirm <?php echo $resultModule['menu_display_name']; ?></h1>
<?php
if (isset($_GET['new']) == 1 && isset($_GET['submit']) == 1 && $resultModule['paid_module'] == 1 && $resultModule['paid_posting'] == 1) {
?>
<p>Successfully added your record in database. Please click below to activate your posting.</p>
<script src="https://www.dwolla.com/scripts/button.min.js" 
    class="dwolla_button" 
    type="text/javascript" 
    data-key="IVQZVEDpwzJiECGzWKczZZ0pUa6TXqMylcJCd3pBTR3IaLWT0h" 
    data-redirect="<?php echo $currentURL; ?>/auto/confirm?id=<?php echo $_GET['id']; ?>&module_id=<?php echo $colname_rsModule; ?>&new=1&confirm=1" 
    data-label="Buy Now (<?php echo $resultModule['paid_amount']; ?>)" 
...
<!--https://developers.dwolla.com/dev/pages/button#redirect
    data-test="true"-->
<?php
} else if (isset($_GET['new']) == 1 && isset($_GET['submit']) == 1) {
