...
$details = $modelGeneral->getDetails('z_history LEFT JOIN google_auth ON z_history.uid = google_auth.uid', 0, $params);
if (empty($details) || empty($details[0]) || $details[0]['uid'] != $_SESSION['user']['id']) {
  header("Location: ".$currentURL.'/history/myhistory');
  exit;
}
$details = $details[0];
if (empty($_POST)) {
...
$history_image = json_decode($details['history_image'], 1);
$history_video = json_decode($details['history_video'], 1);
$history_urls = json_decode($details['history_urls'], 1);
...
if (isset($_POST['MM_Insert']) && $_POST['MM_Insert'] == 'form1') {
  try {
      ...
      $where = sprintf('uid = %s AND history_id=%s', $modelGeneral->qstr($_SESSION['user']['id']), $modelGeneral->qstr($_GET['id']));
      $result = $modelGeneral->updateDetails('z_history', $data, $where);
      if (!empty($data['is_private']))
      ...
      header("Location: ".$currentURL."/history/edit?id=".$_GET['id']."&msg=".urlencode($error));
      exit;
  } catch (Exception $e) {
      $error = $e->getMessage();
  }
...
<form name="form1" id="form1" method="post" action="<?php echo $currentURL."/history/edit?id=".$_GET['id']; ?>" onSubmit="return validateFrm();">
<div class="row">
        <div class="col-md-12">
                        ...
                        <div class="panel-body">
                          ...
                          <div class="form-group" id="history_imgs">
                              ...
                              <?php if (!empty($history_image)) { ?>
                                <?php foreach ($history_image as $imgs) { ?>
                                  <input type="text" name="history_image[]" class="inputText" placeholder="Enter Image URL" value="<?php echo $imgs; ?>" /> 
                                <?php } ?>
                              <?php } else { ?>
                          ...
                          <div class="form-group" id="history_vids">
                              ...
                              <?php if (!empty($history_video)) { ?>
                                <?php foreach ($history_video as $vids) { ?>
                              <input type="text" name="history_video[]" class="inputText" placeholder="Enter Youtube or Vimeo Video URL" value="<?php echo $vids; ?>" />
                                <?php } ?>
                              <?php } else { ?>
                          ...
                          <div class="form-group" id="history_web">
                              <strong>History Video:</strong> <br />
                              <?php if (!empty($history_urls)) { ?>
                                <?php foreach ($history_urls as $urls) { ?>
                              <input type="text" name="history_urls[]" class="inputText" placeholder="Enter Web URL" value="<?php echo $urls; ?>" />
                                <?php } ?>
                              <?php } else { ?>
...
<p>
  ...
  <input type="submit" name="submit" id="submit" value="Update History" class="inputText">
</p>
</form>
