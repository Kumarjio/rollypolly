...
if (!empty($_POST['MM_Insert']) && !empty($_SESSION['_nonce'])) {
  ...
  if (empty($error)) {
    //submit the form
    $url = APIHTTPPATH.'/help/buysell/add';
    $params = $_POST;
    $params['uid'] = $_SESSION['user']['id'];
    $params['city_id'] = $city_id;
    $params['buy_sell_type'] = 'sell';
...
<div id="wizard">
    ...
    <div>
      ...
      <fieldset>
        <legend>posting detials</legend>
        <p>
          <label for="size_dimens"><strong><br>
          size / dimensions:</strong></label>
          <input type="text" name="size_dimens" id="size_dimens">
        </p>
      </fieldset>
    </div>
    <h1>Details</h1>
