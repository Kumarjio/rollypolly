...
function findHoroInfo($kundali, $month, $day, $year, $hour, $minute, $data)
{
    if (empty($month) || empty($day) || empty($year) || empty($data)) {
        return false;
    }
    
    $arr = $kundali->precalculate($month, $day, $year, $hour, $minute, $data['zone_h'], $data['zone_m'], $data['lon_h'], $data['lon_m'], $data['lat_h'], $data['lat_m'], $data['dst'], $data['lon_e'], $data['lat_s']);
    $arr[] = array('month' => $month, 'day' => $day, 'year' => $year, 'hour' => $hour, 'minute' => $minute, 'data' => $data);
