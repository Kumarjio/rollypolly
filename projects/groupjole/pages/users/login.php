...
if (isset($_GET['redirect_url'])) {
    $_SESSION['redirect_url'] = urldecode($_GET['redirect_url']);
}
...
try {
    ...
    if ($client->getAccessToken()) {
        ...
        if (empty($return)) {
            ...
            send_email($arr['email'], 'New Registration for "'.SITENAME.'"', 'register.php', $arr);
...
if (!empty($error)) {
  echo '<div class="error">'.$error.'</div>';
	...
	echo "<a class='login' href='$authUrl'><img src=\"".HTTPPATH."/images/google-login-button-asif18.png\" alt=\"Google login\" title=\"login with google\" /></a>";
	} else {
		?>
