...
class General
{
    ...
    public function getUser($uid, $field='google', $useCache=true, $sitename='')
    {
      $sql = sprintf("SELECT * FROM users WHERE {$field}_id = %s AND {$field}_auth = 1 AND sitename = '%s'", $this->qstr($uid), $sitename);
      if ($useCache)
