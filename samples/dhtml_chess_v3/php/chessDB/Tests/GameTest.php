...
class GameTest extends ChessTests
{
     ...
     * @test
     */
    public function shouldGetDefaultFenWhenCreatingNewGame()
    {
        // given
        $game = new Game();
        // then
        $this->assertEquals($this->newGameFen, $game->getCurrentFen());
    }
    ...
    public function shouldBeAbleToAppendMove()
    {
        ...
        // when
        $move = $game->appendMove('e4');

        // then
        $this->assertEquals('e2', $move['from']);
        $this->assertEquals('e4', $move['to']);
        $this->assertEquals('rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq - 0 1', $move['fen']);
    }
    /**
     * @test
     */
    public function shouldBeAbleToAppendMoveUsingArray()
    {
        // given
        $game = new Game();

        // when
        $move = $game->appendMove(array('from' => 'e2', 'to' => 'e4'));
     ...
     * @test
     */
    public function shouldBeAbleToAppendMoveToExistingGame()
    {
        // given
        $game = new Game($this->getGameWithAMove());
        // when
        $move = $game->appendMove('e5');

        // then
        $this->assertEquals('e7', $move['from']);
        $this->assertEquals('e5', $move['to']);
        $this->assertEquals('rnbqkbnr/pppp1ppp/8/4p3/4P3/8/PPPP1PPP/RNBQKBNR w KQkq - 0 2', $move['fen']);
    }
    ...
    private $newGameFen = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1";
    ...
    private $gameJSON = '{
            ...
            "moves":[
                {"m":"d4","from":"d2","to":"d4","fen":"rnbqkbnr\/pppppppp\/8\/8\/3P4\/8\/PPP1PPPP\/RNBQKBNR b KQkq - 0 1"},
