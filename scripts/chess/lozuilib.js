...
lozData.e.push({
  label  : 'Stockfish 5',
  source : '/scripts/chess/stockfish.js'
});

lozData.e.push({
  label   : 'Lozza',
  source  : '/scripts/chess/lozza.js'
});

lozData.e.push({
  label  : 'P4wn',
  source : '/scripts/chess/p4wn.js'
});
...
function lozEngineList (source) {

  lozData.label  = 'Stockfish 5';
  //lozData.source = '/scripts/chess/lozza.js';
  lozData.source = '/scripts/chess/stockfish.js';

  if (source) {
    for (var i=0; i < lozData.e.length; i++) {
      if (lozData.e[i].source == source) {
        lozData.label  = lozData.e[i].label;
        lozData.source = lozData.e[i].source;
        break;
      }
    }
  }
  ...
  h += '<button type="button" class="btn btn-danger">' + lozData.label + '</button>';
  h += '<button type="button" class="btn btn-danger dropdown-toggle" data-toggle="dropdown" aria-expanded="false">';
  h += '<span class="caret"></span>';
  h += '<span class="sr-only">Toggle Dropdown</span>';
  h += '</button>';
  h += '<ul class="dropdown-menu" role="menu">';

  for (var i=0; i < lozData.e.length; i++)
    h += '<li><a class="selengine" id="' + lozData.e[i].source + '" href="javascript:;">' + lozData.e[i].label + '</a></li>';

  //h += '<li class="divider"></li>';
  //h += '<li><a target="_blank" href="engines.htm">Engine info</a></li>';
  h += '</ul>';
  h += '</div>';
