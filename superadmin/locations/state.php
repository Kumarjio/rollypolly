...
$query_rsLocation = sprintf("SELECT geo_states.*, geo_countries.name as country, geo_countries.country_owner_id  FROM geo_states LEFT JOIN geo_countries ON geo_states.con_id = geo_countries.con_id WHERE sta_id = %s", GetSQLValueString($colname_rsLocation, "int"));
...
mysql_select_db($database_connMain, $connMain);
$query_rsBids = sprintf("SELECT * FROM location_bids WHERE location_id = %s AND location_type = 'State' AND period_bids = 'Jan 2015 To Dec 2015' ORDER BY bid_amount DESC", GetSQLValueString($colname_rsBids, "int"));
